...
class DepthWiseConv(nn.Module):
    """Depthwise convolution used in the Tokenized MLP Stage
    In the code, this block is grouped together with the shifted MLP"""
...
class OverlapPatchEmbed(nn.Module):
    """
    Since UNext uses tokenized MLP (the idea is similar to ViTs), we need to convert the image into patches before passing it to the MLP, the same way ViTs do.
    """
...
class ShiftMLP(nn.Module):
    def __init__(
        ...
        self.DepthWiseConv = DepthWiseConv(hidden_features)
        self.act = nn.GELU()
