...
# Path to Tesseract OCR (Only needed on Windows)
...
def extract_text(image):
    ...
    # Apply thresholding (optional, improves clarity)
...
def clean_ocr_output(ocr_output):
    """
    Cleans and concatenates OCR output into a readable sentence.
    
    Parameters:
        ocr_output (list): List of text fragments from EasyOCR.
    
    Returns:
        str: Cleaned and formatted text.
    """
    # Step 1: Join all fragments into a single string
    text = " ".join(ocr_output)

    # Step 2: Fix common OCR artifacts
    text = text.replace("_", "")  # Remove underscores
    text = text.replace(" ,", ",").replace(" .", ".")  # Fix space before punctuation
    text = re.sub(r'\s+', ' ', text).strip()  # Remove extra spaces

    # Step 3: Capitalization and basic sentence correction
    text = re.sub(r'(\w)([A-Z])', r'\1. \2', text)  # Add a period before capitalized words if missing
    text = text[0].upper() + text[1:]  # Ensure first letter is capitalized
...
if uploaded_file:
    ...
    # **New "Extract Text" Button**
    if st.button("Extract Text"):
        with st.spinner("Extracting text..."):
            extracted_text = clean_ocr_output(extract_text(extracted_image))
